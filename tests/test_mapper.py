...
from tha_map_runner import MapperError, enrich_rows


def test_basic_mapping(rows, json_items, mapping) -> None:
    result = enrich_rows(rows, json_items, mapping, row_key="Org BK", source_key="sourcedId")
...
def test_input_not_mutated(rows, json_items, mapping) -> None:
    ...
    enrich_rows(rows, json_items, mapping, row_key="Org BK", source_key="sourcedId")
...
def test_no_match_error_sets_row_status(rows, json_items, mapping) -> None:
    result = enrich_rows(
        rows, json_items, mapping, row_key="Org BK", source_key="sourcedId", on_no_match="error"
    )
    no_match = result[2]
...
def test_no_match_blank_adds_empty_columns(rows, json_items, mapping) -> None:
    result = enrich_rows(
        rows, json_items, mapping, row_key="Org BK", source_key="sourcedId", on_no_match="blank"
    )
    no_match = result[2]
    assert no_match["Org Name"] == ""
    assert no_match["Parent BK"] == ""
...
def test_invalid_on_no_match_raises(rows, json_items, mapping) -> None:
    with pytest.raises(MapperError, match="on_no_match"):
        enrich_rows(
            rows, json_items, mapping, row_key="Org BK", source_key="sourcedId", on_no_match="bad"
        )


# --- allow_empty_source ---

def test_empty_source_raises_by_default(rows, mapping) -> None:
    with pytest.raises(MapperError, match="empty"):
        enrich_rows(rows, [], mapping, row_key="Org BK", source_key="sourcedId")


def test_empty_source_allowed(rows, mapping) -> None:
    result = enrich_rows(
        rows, [], mapping, row_key="Org BK", source_key="sourcedId", allow_empty_source=True
    )
    assert len(result) == len(rows)
...
def test_error_rows_skipped_by_default(json_items, mapping) -> None:
...
def test_warning_rows_skipped_by_default(json_items, mapping) -> None:
...
def test_custom_skip_statuses(json_items, mapping) -> None:
    rows = [{"Org BK": "school-001", "row status": "pending"}]
    result = enrich_rows(
        rows, json_items, mapping,
        row_key="Org BK", source_key="sourcedId",
        skip_statuses=["pending"],
    )
    assert "Org Name" not in result[0]


def test_empty_skip_statuses_processes_all(json_items, mapping) -> None:
    rows = [{"Org BK": "school-001", "row status": "error", "message": "bad row"}]
    result = enrich_rows(
        rows, json_items, mapping,
        row_key="Org BK", source_key="sourcedId",
...
def test_duplicate_source_key_warns(rows, mapping) -> None:
    dupes = [
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        result = enrich_rows(rows, dupes, mapping, row_key="Org BK", source_key="sourcedId")
...
def test_empty_rows_returns_empty(json_items, mapping) -> None:
    result = enrich_rows([], json_items, mapping, row_key="Org BK", source_key="sourcedId")
    assert result == []


def test_missing_path_in_source_gives_empty_string(rows, mapping) -> None:
    source = [{"sourcedId": "school-001", "name": "Lincoln"}]  # no "parent" key
    result = enrich_rows(rows[:1], source, mapping, row_key="Org BK", source_key="sourcedId")
